...
Accepted; amended by ADR 0027, which carries the first and later Run Headline
updates through the Run Plan reporting tool. Glossary term **Run Headline**
(CONTEXT.md); amends the Peek Card's live title (ADR 0021).
...
- **The echo is the fallback, not the title**: it stands in until the
  ...
  last good one and never fails the Run, following the same
  degrade-without-failing discipline as continuity validation.
...
- The headline travels in a model tool call riding any Tool Round — native tool
  calls are the one channel every round already speaks, so a steering
  correction's next round can revise the title; the pipeline re-emits changes
  as a `run_headline` detail event (no history entry, no Feed Entry). ADR 0027
  combines this with Effort Tier declaration in the Run Plan reporting tool.
