...
What made an Observation — an Action Outcome, a page read, a Look, a Subagent
Report, the user, or a directive. Two Producers observing one settled page
state observe different material; one Producer observing it twice does not.
_Avoid_: tool, source
...
**Search Loop**:
A run flailing blind — consecutive searches with nothing opened between them,
whatever their terms. A search after a search continues the loop; the terms
...
different engine, or the site's own search box — and are one Approach to the
no-progress accounting. A Search Loop no longer asks whether its searches
share an intent, only whether anything was opened between them. A search
that is nothing but scope keeps the scope as its intent, so that it still
counts as a search.
...
It is written only when a developer opts in; a deployed Kiosk writes none of
it.
