...
- record_evidence checkpoints grounded Observations into Session Evidence, where they survive this run ending and serve the whole Session. Web findings cite the source_url you observed and a verbatim excerpt. The user's own words — the command, an ask_user answer, a steering directive — are checkpointed with kind "user" and their exact text. record_candidate records what you are weighing (an option, an item, an identification) and decides it — accepted, rejected, or superseded — citing the memory-N ids of supporting Observations. Checkpoint the user's corrections and your Candidate eliminations so rejected options do not reappear later in the Session.
