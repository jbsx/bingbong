...
/**
 * A session wired to a fake clock: `starts()` counts boundary
 * announcements, `advance` drives the eager-lapse timer (ADR 0005).
 */
function lapseHarness(options?: { windowMs?: number }) {
  ...
  return { session, advance, starts: () => starts }
}
...
describe('sessionMemory', () => {
  ...
  describe('eager lapse', () => {
    it('announces the boundary on a timer once the window elapses while idle', () => {
      const { session, advance, starts } = lapseHarness()
      feed(session.run(), runEvents('find a pizza place', 'Found two.', 1_000))
    ...
    it('announces lazily still when the command beats the timer to the same boundary', () => {
      ...
      const { session, starts } = lapseHarness()
      feed(session.run(), runEvents('find a pizza place', 'Found two.', 1_000))
      const next = session.run()
    ...
    it('honours a custom window for the eager boundary', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: 5_000 })
      feed(session.run(), runEvents('find a pizza place', 'Found two.', 1_000))
    ...
    it('stays silent while the store is empty — nothing to lapse', () => {
      const { advance, starts } = lapseHarness()
      advance(SESSION_WINDOW_MS * 3)
      expect(starts()).toBe(0)
    })
  })

  // Boot-armed lapse (#73): a restart within the Session Window hydrates
  // the view, so the eager-Lapse timer must arm from the hydrated
  // last-run finish — the wipe happens on schedule without a live run
  // first. The model's thread stays fresh on restart (ADR 0005's accepted
  // asymmetry): only the timer arms, nothing joins the thread.

  describe('boot-armed lapse', () => {
    const BOOT_FINISH = 5_000
    const BOOT_WINDOW = 10_000

    it('announces the boundary on a timer armed from the hydrated finish — no live run needed', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)

      advance(BOOT_FINISH + BOOT_WINDOW - 1)
      expect(starts()).toBe(0)
      advance(BOOT_FINISH + BOOT_WINDOW)
      expect(starts()).toBe(1)
    })

    it('arms late in the window: only the remaining time is waited out', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)

      advance(BOOT_FINISH + 8_000)
      expect(starts()).toBe(0)
      advance(BOOT_FINISH + BOOT_WINDOW)
      expect(starts()).toBe(1)
    })

    it('keeps the thread fresh — a command after boot reads no hydrated turns', () => {
      const { session } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)

      const next = session.run()
      next.event({ type: 'command', turnId: TURN, text: 'hello again', at: BOOT_FINISH + 1_000 })
      expect(session.history()).toEqual([])
    })

    it('a live command cancels the boot-armed boundary; its own done re-arms from the live finish', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)

      const run = session.run()
      run.event({ type: 'command', turnId: TURN, text: 'keep working', at: BOOT_FINISH + 1_000 })
      advance(BOOT_FINISH + BOOT_WINDOW + 5_000)
      expect(starts()).toBe(0)

      feed(run, [
        { type: 'display', text: 'Work complete.', at: BOOT_FINISH + 7_000 },
        { type: 'done', turnId: TURN, outcome: 'done', at: BOOT_FINISH + 7_003 },
      ])
      advance(BOOT_FINISH + 7_003 + BOOT_WINDOW - 1)
      expect(starts()).toBe(0)
      advance(BOOT_FINISH + 7_003 + BOOT_WINDOW)
      expect(starts()).toBe(1)
    })

    it('does not announce twice for one boot lapse — the command that follows stays silent', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)
      advance(BOOT_FINISH + BOOT_WINDOW)
      expect(starts()).toBe(1)
      ...
      const next = session.run()
      next.event({ type: 'command', turnId: TURN, text: 'fresh start', at: BOOT_FINISH + BOOT_WINDOW + 1_000 })
      expect(starts()).toBe(1)
    })

    it('a model-invoked clear cancels the boot-armed boundary', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)
      session.clear()

      advance(BOOT_FINISH + BOOT_WINDOW + 10_000)
      expect(starts()).toBe(0)
    })

    it('ignores the boot anchor once the thread is live — the newest exchange owns the boundary', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      feed(session.run(), runEvents('find a pizza place', 'Found two.', 1_000))
      session.armBootLapse(BOOT_FINISH)

      // The live thread's finish (1_003) arms the boundary, not the stale
      // boot anchor: a late armBootLapse cannot push the wipe out.
      advance(1_003 + BOOT_WINDOW - 1)
      expect(starts()).toBe(0)
      advance(1_003 + BOOT_WINDOW)
      expect(starts()).toBe(1)
    })

    it('ignores the boot anchor while a run is live — the view is never wiped mid-run', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      const run = session.run()
      run.event({ type: 'command', turnId: TURN, text: 'still working', at: 1_000 })
      session.armBootLapse(BOOT_FINISH)

      advance(BOOT_FINISH + BOOT_WINDOW + 5_000)
      expect(starts()).toBe(0)

      // The live run's own done arms the real boundary.
      feed(run, [
        { type: 'display', text: 'Work complete.', at: 20_000 },
        { type: 'done', turnId: TURN, outcome: 'done', at: 20_003 },
      ])
      advance(20_003 + BOOT_WINDOW)
      expect(starts()).toBe(1)
    })

    it('dispose cancels the boot-armed boundary without announcing — a closed window never fires', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      session.armBootLapse(BOOT_FINISH)
      session.dispose()
    ...
    it('dispose cancels a live thread\'s pending boundary too', () => {
      const { session, advance, starts } = lapseHarness({ windowMs: BOOT_WINDOW })
      feed(session.run(), runEvents('find a pizza place', 'Found two.', 1_000))
      session.dispose()

      advance(1_003 + BOOT_WINDOW + 10_000)
      expect(starts()).toBe(0)
    })
  })
})
