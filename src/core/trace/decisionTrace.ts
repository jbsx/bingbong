...
export interface DecisionEventInput<Q extends DecisionQuestions> {
  ...
  /** `act`: the seam acts on an answer that clears; `shadow`: it only records. */
  readonly mode: 'act' | 'shadow'
  readonly agentId?: string
}

/** What a seam does with a result: the one derivation both the record and the seam read. */
export function decisionActed<Q extends DecisionQuestions>(result: DecisionResult<Q>, mode: 'act' | 'shadow', threshold: DecisionThresholds): DecisionActed {
