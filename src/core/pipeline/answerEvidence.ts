// The displayed Answer's evidence grounding (#122, ADR 0028): source
// links derived from the Session Evidence the Answer cites, and the
// hard rule that internal identities — Memory Entry ids, Run
// Observation ids — never reach the user. The spoken line and the
...
/**
 * The text the dashboard shows (#122): the model's display with
 * internal identity tokens scrubbed, plus a Sources line carrying the
 * derived links the text does not already include. Clean text over no
 * sources passes through unchanged.
 */
export function displayedAnswerText(display: string, sources: readonly MemoryReference[]): string {
  const scrubbed = display
    ...
    .replace(/\([ \t]*,[ \t]*\)/g, '()')
    .replace(/\[[ \t]*,[ \t]*\]/g, '[]')
  const missing = sources.filter((source) => !scrubbed.includes(source.url))
